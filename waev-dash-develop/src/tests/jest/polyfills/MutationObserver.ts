 ...
 * @see https://github.com/aurelia/pal-nodejs/blob/master/src/polyfills/mutation-observer.ts
 * @see https://github.com/thymikee/jest-preset-angular/issues/63
 */
...
export class Util {
  ...
  public static expando = 'mo_id';

  public static clone($target, config) {
   ...
   * @return {number}
   */
  public static indexOfCustomNode(set, $node, idx) {
  ...
  /**
   * Attempt to uniquely id an element for hashing. We could optimize this for
   * legacy browsers but it hopefully wont be called enough to be a concern
   *
   * @param {Node} $ele
   * @return {(string|number)}
   */
  public static getElementId($ele) {
    try {
      // tslint:disable-next-line:no-static-this
   ...
   */
  public static map(set, iterator) {
    const results = [];
   ...
   */
  public static reduce(set, iterator, memo) {
    for (let index = 0; index < set.length; index++) {
   ...
   */
  public static indexOf(set, item, idx, prop?) {
...
export class MutationObserver {
  ...
  private searchSubtree(mutations, $target, $oldstate, config) {
    ...
    const _findMutations = (node, old) => {
      ...
      // we delay the intialization of these for marginal performance in the
      // expected case (actually quite signficant on large subtrees when these would be otherwise unused)
      ...
      while (i < klen || j < olen) {
        ...
        if ($cur === $old) {
          ...
          if (
            $old &&
            // special case: the changes may have been resolved: i and j appear
            // congurent so we can continue using the expected case
            $old !== $kids[i]
          ) {
...
export class MutationNotifier extends EventEmitter {
  ...
  public notifyChanged(node: Node) {
