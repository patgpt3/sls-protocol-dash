...
function collapseItem(theme: Theme, ownerState: any, isName: boolean) {
  const { palette, transitions, breakpoints, boxShadows, borders, functions } = theme;
  const { active, transparentSidenav, whiteSidenav, darkMode, isMiniSidenav } = ownerState;
  ...
  return {
    ...
    paddingLeft: isName && (!isMiniSidenav ? '2px' : `${pxToRem(8)} ${pxToRem(16)}`),
...
function collapseText(theme: any, ownerState: any) {
  ...
  const { isMiniSidenav, transparentSidenav, active } = ownerState;
  ...
  return {
    ...
    [breakpoints.up('xl')]: {
      opacity: isMiniSidenav || (isMiniSidenav && transparentSidenav) ? 0 : 1,
      maxWidth: isMiniSidenav || (isMiniSidenav && transparentSidenav) ? 0 : '100%',
      marginLeft: isMiniSidenav || (isMiniSidenav && transparentSidenav) ? 0 : pxToRem(10),
